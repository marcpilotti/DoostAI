...
type Platform = "instagram" | "google" | "linkedin";
...
/** Reel-style ad card (9:16 vertical) */
function ReelAdCard({
  ad,
  brand,
  label,
    ...
    >
      ...
      >
        ...
        <div className="absolute inset-x-0 bottom-0 z-10 flex flex-col gap-1.5 p-4">
          ...
          {/* CTA */}
          <span
            className="mt-1 inline-flex self-start items-center gap-1 rounded-lg px-3 py-1.5 text-[10px] font-bold"
        ...
        for (const line of lines) {
          ...
          try {
            ...
            if (d.event === "complete" && d.result?.copies) {
              ...
              setAds(
                d.result.copies.map((c: Record<string, string>, i: number) => ({
                  id: `ad-${Date.now()}-${i}`,
                  platform: selectedPlatforms[0],
